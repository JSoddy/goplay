...
const fileToken string = "d:\\James\\Temp\\Knapsack1.txt"
...
func knapsackLoops(itemList []item, knapSize int) int {
	...
	for i := 1; i < len(solver); i++ {
		for j := 1; j < len(solver[i]); j++ {
			if j < itemList[i-1].weight {
				solver[i][j]	= solver[i-1][j]
			} else {
				solver[i][j]	= int(math.Max(float64(solver[i-1][j]), 
									float64(solver[i-1][j-itemList[i-1].weight] + itemList[i-1].value)))
			}
		}
	}
...
func newSolver(itemList []item, knapSize int) [][]int {
	solver 		:= make([][]int, len(itemList)+1)

...
func (cs *ConsoleStats) parse(statsStringSlice []string) {
	...
	// Assign parsed data to struct
	cs.name = name
	cs.yearAppeared = uint(yearAppeared)
...
func makeNinStats(statsStringSlice []string) NinStats {
	...
	// Convert strings to other types as necessary
	var newNinStats [1]NinStats

	var newConsoleStats ConsoleStats
	(&newConsoleStats).parse(statsStringSlice)
	newNinStats[0] = &newConsoleStats

	return newNinStats[0]
}
...
func askTrivia(infoArray []NinStats) {
	...
	infoArray[elementNumber].poseTrivia()
	// ^^ For now just ask for the year

}

func (cs *ConsoleStats) poseTrivia() {
	var userAnswer uint
	// Pose the question that has been chosen
	fmt.Println("In what year was", cs.name, "released?")
	fmt.Scanln(&userAnswer)
	for userAnswer < 1980 || userAnswer > 2015 {
		fmt.Println("Please enter a year between 1980 and 2015.")
		fmt.Scanln(&userAnswer)
	}
	if userAnswer == cs.yearAppeared {
		fmt.Println("Correct! The", cs.name, "was released in", cs.yearAppeared)
	} else {
		fmt.Println("Wrong! The", cs.name, "was released in", cs.yearAppeared)
	}
}
